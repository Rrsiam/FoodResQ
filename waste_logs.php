...
// Fetch waste logs with user names
$logs_query = $conn->query("SELECT wl.*, u.name as user_name, wc.category_name 
                           FROM waste_log wl 
                           LEFT JOIN users u ON wl.user_id = u.user_id
                           LEFT JOIN waste_categories wc ON wl.category_id = wc.category_id");
$logs = $logs_query->fetch_all(MYSQLI_ASSOC);
?>
...
<head>
  <meta charset="UTF-8">
  <title>Waste Logs - FoodResQ</title>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= $log['category_name'] ?? 'N/A' ?></td>
                    <td><?= $log['status'] ?></td>
                    <td>
                        <a href="view_waste.php?id=<?= $log['waste_id'] ?>" class="btn-action btn-view">View</a>
                        <a href="edit_waste.php?id=<?= $log['waste_id'] ?>" class="btn-action btn-edit">Edit</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
