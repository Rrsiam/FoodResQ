...
// Fetch processing plants with location info
$plants_query = $conn->query("SELECT pp.*, l.address 
                             FROM processing_plants pp 
                             JOIN locations l ON pp.location_id = l.location_id");
$plants = $plants_query->fetch_all(MYSQLI_ASSOC);
?>
...
<head>
  <meta charset="UTF-8">
  <title>Processing Plants - FoodResQ</title>
...
<body>
    ...
    <div class="main-content">
        <div class="page-title">
            <h1>Processing Plants</h1>
            <p>Welcome, <?= $admin_name ?></p>
        </div>

        <div class="action-buttons">
            <a href="add_plant.php" class="btn-submit">Add New Plant</a>
        </div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?= htmlspecialchars($plant['address']) ?></td>
                    <td><?= $plant['processing_type'] ?></td>
                    <td><?= $plant['capacity'] ?></td>
                    <td>
                        <a href="edit_plant.php?id=<?= $plant['plant_id'] ?>" class="btn-action btn-edit">Edit</a>
                        <a href="delete_plant.php?id=<?= $plant['plant_id'] ?>" class="btn-action btn-delete" onclick="return confirm('Are you sure?')">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
