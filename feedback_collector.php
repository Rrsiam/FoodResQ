...
$collector_query = $conn->query("SELECT name FROM users WHERE user_id = '$collector_id' AND user_type = 'Collector'");
$collector_data = $collector_query->fetch_assoc();
$collector_name = $collector_data ? $collector_data['name'] : 'Collector User';
...
if ($editing_id !== null) {
    $edit_query = $conn->query("SELECT * FROM feedback WHERE feedback_id = '$editing_id' AND user_id = '$collector_id'");
    $edit_feedback_data = $edit_query->fetch_assoc();
}

// Delete feedback and related votes
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    $del_id = intval($_GET['delete']);
    $conn->query("DELETE FROM feedback_ratings WHERE feedback_id = '$del_id'");
    $conn->query("DELETE FROM feedback WHERE feedback_id = '$del_id' AND user_id = '$collector_id'");
    header("Location: feedback_collector.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_feedback'])) {
    ...
    if (strlen($comments) < 3) {
        ...
        $conn->query("UPDATE feedback SET comments = '$comments' WHERE feedback_id = '$feedback_id' AND user_id = '$collector_id'");
        header("Location: feedback_collector.php");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_feedback'])) {
    ...
    if (stripos($comments, 'fuck') !== false || strlen($comments) < 3) {
        ...
        $conn->query("INSERT INTO feedback (user_id, date, comments) VALUES ('$collector_id', CURDATE(), '$comments')");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['vote_feedback'])) {
    ...
    $check->bind_param("ii", $feedback_id, $collector_id);
    $check->execute();
    $result = $check->get_result();

    if ($result->num_rows == 0) {
        $stmt = $conn->prepare("INSERT INTO feedback_ratings (feedback_id, rater_id, helpful) VALUES (?, ?, ?)");
        $stmt->bind_param("iii", $feedback_id, $collector_id, $vote);
        $stmt->execute();
    }
    header("Location: feedback_collector.php");
    exit();
...
<div class="sidebar">
    <ul>
        <li><a href="dashboard_collector.php"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
        <li><a href="collection_schedule_collector.php"><i class="fas fa-calendar"></i> My Schedule</a></li>
        <li><a href="alerts_collector.php"><i class="fas fa-bell"></i> Alerts</a></li>
        <li class="active"><a href="feedback_collector.php"><i class="fas fa-comment"></i> Feedback</a></li>
    ...
    <div id="my-feedback" class="tab-content <?= $active_tab === 'my-feedback' ? 'active' : '' ?>">
        ...
        <?php if ($edit_feedback_data): ?>
            <form method="POST" class="feedback-form">
                ...
                <a href="feedback_collector.php" class="btn-submit" style="margin-left: 10px;">Cancel</a>
            </form>
        <?php endif; ?>
        ...
        <?php while ($feedback = $my_feedback_result->fetch_assoc()): ?>
            ...
            <div class="feedback-container">
                ...
                <div class="feedback-actions">
                    <a href="feedback_collector.php?edit=<?= $feedback['feedback_id'] ?>" class="btn-submit btn-edit">Edit</a>
                    <a href="feedback_collector.php?delete=<?= $feedback['feedback_id'] ?>" class="btn-submit btn-delete" onclick="return confirm('Delete this feedback?');">Delete</a>
                </div>
            </div>
        <?php endwhile; ?>
    </div>
